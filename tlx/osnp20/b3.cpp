...
typedef struct{
    int mulai, akhir, upah;
} Kerjaan;
void solve(){
    int n; cin >> n;
    vector<Kerjaan> vec(n);

    forv(n) cin >> vec[i].mulai >> vec[i].akhir >> vec[i].upah;

    sort(all(vec), [](const Kerjaan &a, const Kerjaan &b){
        if(a.mulai != b.mulai) return a.mulai < b.mulai;
        if(a.akhir != b.akhir) return a.akhir < b.akhir;
        return a.upah < b.upah;
    });
