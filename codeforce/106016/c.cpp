...
// ===============================================================
bool mungkin(ll mid, ll k, ll n, vector<pll>& v){
    ll butuh = 0;
    for(int i = 0; i < n; ++i){
        ll ltd = mid / v[i].se;

        if(v[i].fi > ltd) butuh += v[i].fi - ltd;

        if(butuh > k) return false;
        return butuh <= k;
    }
}
...
void solve(){
    ...
    cin >> n >> k;
    vector<pll> bayar(n);
    forv(n) cin >> bayar[i].fi;
    forv(n) cin >> bayar[i].se;
